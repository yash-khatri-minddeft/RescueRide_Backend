	...
	jwt.verify(token, process.env.JWT_SECRETKEY, async (err, decode) => {
		if (err) {
			...
			if (password === cPassword) {
				...
				const getCtrl = await controller.findOneAndUpdate({ _id: userId }, { password: hashedPassword });
				res.status(200).send({ success: true, message: 'Password changed successfully!', token: token })
			} else {
				res.status(200).send({ success: false, error: `Password didn't matched!` })
			}
		}
	})
}
